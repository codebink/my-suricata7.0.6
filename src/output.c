...
TAILQ_HEAD(, OutputFileRolloverFlag_) output_file_rotation_flags = TAILQ_HEAD_INITIALIZER(output_file_rotation_flags);
...
/**
 * 下述以OutputRegiste开头的函数，均为注册一个输出模块。
 * 内部实现均为先申请一个OutputModule变量，然后为其成员赋值，并将其插入到output_modules列表中。
 * 不同函数区别：为OutputModule变量成员变量的赋值过程不同。
 */
                                ...
                                ThreadInitFunc ThreadInit, ThreadDeinitFunc ThreadDeinit, ThreadExitPrintStatsFunc ThreadExitPrintStats)
{
    ...
    /* 申请一个日志模块，然后加入到全局变量output_modules 里面。
     * 后面会通过RunModeInitializeEveOutput函数，循环获取output_modules内容填充到对应根日志的list中去
     */
    OutputModule *module = SCCalloc(1, sizeof(*module));
    if (unlikely(module == NULL))
    {
    ...
    module->name = name;
    module->conf_name = conf_name;
    module->InitFunc = InitFunc;    // 设置初始化函数
...
void OutputRegisterPacketSubModule(LoggerId id, const char *parent_name, const char *name, const char *conf_name, 
                                            OutputInitSubFunc InitFunc, PacketLogger PacketLogFunc, PacketLogCondition PacketConditionFunc,
                                            ThreadInitFunc ThreadInit, ThreadDeinitFunc ThreadDeinit, ThreadExitPrintStatsFunc ThreadExitPrintStats)
{
    if (unlikely(PacketLogFunc == NULL || PacketConditionFunc == NULL))
    {
    ...
    module->name = name;
    module->conf_name = conf_name;
    module->parent_name = parent_name;  // 设置父节点名称
    module->InitSubFunc = InitFunc;     // 设置子模块初始化函数
    module->PacketLogFunc = PacketLogFunc;
    module->PacketConditionFunc = PacketConditionFunc;
    module->ThreadInit = ThreadInit;
    module->ThreadDeinit = ThreadDeinit;
...
static void OutputRegisterTxModuleWrapper(  LoggerId id, const char *name, const char *conf_name, 
                                                    OutputInitFunc InitFunc, AppProto alproto, TxLogger TxLogFunc, int tc_log_progress, int ts_log_progress, TxLoggerCondition TxLogCondition,
                                                    ThreadInitFunc ThreadInit, ThreadDeinitFunc ThreadDeinit, ThreadExitPrintStatsFunc ThreadExitPrintStats)
{
 ...
 * 返回值：成功时返回 0，失败时返回 -1。
 */
static void OutputRegisterTxSubModuleWrapper(  LoggerId id, const char *parent_name, const char *name, const char *conf_name,
                                                        OutputInitSubFunc InitFunc, AppProto alproto, TxLogger TxLogFunc, int tc_log_progress, int ts_log_progress, TxLoggerCondition TxLogCondition,
                                                        ThreadInitFunc ThreadInit, ThreadDeinitFunc ThreadDeinit, ThreadExitPrintStatsFunc ThreadExitPrintStats)
{
    if (unlikely(TxLogFunc == NULL))
    {
...
void OutputRegisterTxModuleWithCondition( LoggerId id, const char *name,  const char *conf_name,
                                                    ...
                                                    ThreadInitFunc ThreadInit, ThreadDeinitFunc ThreadDeinit, ThreadExitPrintStatsFunc ThreadExitPrintStats)
{
    OutputRegisterTxModuleWrapper(  id, name, conf_name, 
                                    InitFunc, alproto, TxLogFunc, -1, -1, TxLogCondition, 
...
{
    OutputRegisterTxSubModuleWrapper(id, parent_name, name, conf_name, 
                                     InitFunc, alproto, TxLogFunc, tc_log_progress, ts_log_progress, NULL,
                                     ThreadInit, ThreadDeinit, ThreadExitPrintStats);
}
 ...
 * 返回值： 成功时返回 0，失败时返回 -1。
 */
void OutputRegisterFiledataSubModule(LoggerId id, const char *parent_name, const char *name, const char *conf_name, 
 ...
 * 功能：根据名称获取输出模块。
 *
 * 返回值：指定名称的 OutputModule，如果未注册指定名称的输出模块，则返回 NULL。
 */
 ...
 * 功能：注册文件轮换通知标志。
 *
 * 参数 flag：一个指针，当请求文件轮换时，该指针将被设置为1。
 */
 ...
 * 功能：取消注册文件轮换标志。
 *
 * 请注意，使用尚未注册的标志调用此函数是安全的，在这种情况下，此函数不会执行任何操作。
 *
 * 参数 flag：一个先前已注册用于文件轮换通知的指针。
 */
 ...
 * 日志输出总接口
 *
 * 遍历RootLoggers调用相关大类的日志函数，大类日志函数遍历内部的list循环写日志
 * list的循环其实是注册具体日志的具体日志模块转变得到的大类日志
 *
 * 只有三处被调用：
 *  flow-worker.c: FlowWorker
 *  flow-worker.c: FlowWorkerStreamTCPUpdate
 *  flow-worker.c: FlowWorkerFlowTimeout
 */
TmEcode OutputLoggerLog(ThreadVars *tv, Packet *p, void *thread_data)
{
    ...
    /* RootLoggers就是全局的大类日志，循环调用每个大类的日志输出 */    
    ...
    TAILQ_FOREACH(logger, &active_loggers, entries)
    {
        ...
        if (logger->ThreadInit != NULL)
        {
            if (logger->ThreadInit(tv, initdata, &child_thread_data) == TM_ECODE_OK)
            {
                ...
                {
                    /* Undo everything, calling de-init will take careof that. */
...
/* 将大类的日志注册到rootLogger上 */
void OutputRegisterRootLogger(        ThreadInitFunc ThreadInit, ThreadDeinitFunc ThreadDeinit, ThreadExitPrintStatsFunc ThreadExitPrintStats,
...
/** 
 * 设置活跃的大类日志
 * 遍历已注册的大类日志，如果该大类日志中活跃的非大类日志的个数大于0，则将大类日志注册到活跃的大类日志列表中
 */
 ...
 */
/**
 * 功能：注册所有root日志，添加到registered_loggers列表中
 * 
 * 说明：suricata的日志先按照数据包(packet)日志、文件(filedata和filelogger)日志、事务(tx)日志、流(streaming)日志进行分类
 */
...
void OutputRegisterLoggers(void)
{
    ...
    /* kingbase */
    JsonKingbaseLogRegister();
	
    /* cassandra */
    JsonCassandraLogRegister();
	
    /* radius */
    JsonRadiusLogRegister();
	
    /* redis */
    JsonRedisLogRegister();
	
    /* mongodb */
    JsonMongodbLogRegister();
	
    /* krb5 TCP */
    JsonKrb5LogRegisterTcp();

    /* krb5 UDP */
    JsonKrb5LogRegisterUdp();

    /* ldap */
    JsonLdapLogRegister();
	
    /* gitsmart */
    JsonGitsmartLogRegister();
	
    /* sqlserver */
    JsonSqlserverLogRegister();

    /* oracle */
    JsonOracleLogRegister();
