...
static void StatsInitCtxPreOutput(void)
{
    ...
    if (stats != NULL) 
    {
        ...
        if (gstats == NULL)
        {
            SCLogWarning("global stats config is missing. Stats enabled through legacy stats.log. See %s/configuration/suricata-yaml.html#stats", GetDocURL());
        }
        ...
        if (interval != NULL)
        {
            ...
            {
                SCLogWarning("Invalid value for interval: \"%s\". Resetting to %d.", interval, STATS_MGMTT_TTS);
