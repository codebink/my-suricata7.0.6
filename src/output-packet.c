...
/* 此模块的每个线程数据，包含packet loggers的每个线程的数据列表 */
 ...
 * it's perfectly valid that have multiple instances of the same log module (e.g. fast.log) with different output ctx'. 

 */
/*
 * logger实例，一个模块 + 一个输出ctx，
 * 具有不同输出ctx的相同日志模块（例如fast.log）的多个实例是完全有效的。
 */
...
int OutputRegisterPacketLogger(         LoggerId logger_id, const char *name, PacketLogger LogFunc, PacketLogCondition ConditionFunc, OutputCtx *output_ctx,
...
static TmEcode OutputPacketLog(ThreadVars *tv, Packet *p, void *thread_data)
{
    ...
    /**
     * 关键是从list这个静态链表中取日志，然后输出
     *
     * 那么这个静态的链表的值在哪里赋值的呢？
     * 1) 通过OutputRegisterLoggers函数中非rootLogger的日志注册，注册具体的日志到output_modules模块中；
     * 2) 在初始化的时候，通过RunModeInitializeEveOutput和SetupOutput函数将output_modules 变成串到相关大类的list里面去
     */
    if (list == NULL)
    {
        /* No child loggers. */
 ...
 *  This will run the thread init functions for the individual registered loggers */
/** 功能: packet日志线程初始化
 *  这将为单个注册的日志运行线程初始化函数
 */
...
{
    OutputRegisterRootLogger(   OutputPacketLogThreadInit, OutputPacketLogThreadDeinit, OutputPacketLogExitPrintStats,
