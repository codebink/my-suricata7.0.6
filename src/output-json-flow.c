...
static JsonBuilder *CreateEveHeaderFromFlow(const Flow *f)
{
    ...
    if (f->livedev) {
        ...
        jb_set_string(jb, "in_iface", flow_conf.in_iface);
    }
...
void EveAddFlow(Flow *f, JsonBuilder *js)
{
	...
	int u_time_set = 1;
	if (1 == u_time_set && 0 < strlen(timebuf1)) {
		/* 2017-05-11T18:32:18.358953+0800 格式转换成 tv 结构 */
		ret = ParseTimestamp(timebuf1, strlen(timebuf1), &tv, &timezone_offset);
		if (0 == ret) {
			/* 这里已经取出	 精确到微妙的时间 */
			timestamp = (tv.tv_sec * 1000000) + tv.tv_usec;

			/* 微妙时间戳转换成 string */
			len = snprintf(u_time, sizeof(u_time), "%lu", timestamp);
			if (strlen(u_time) == (size_t)len) {
				jb_set_string(js, "start", u_time);
			}else {
...
static void EveFlowLogJSON(OutputJsonThreadCtx *aft, JsonBuilder *jb, Flow *f)
{
	...
	int u_time_set = 1;
	if (1 == u_time_set && 0 < strlen(timebuf2)) {
		/* 2017-05-11T18:32:18.358953+0800 格式转换成 tv 结构 */
		ret = ParseTimestamp(timebuf2, strlen(timebuf2), &tv, &timezone_offset);
		if (0 == ret) {
			/* 这里已经取出	 精确到微妙的时间 */
			timestamp = (tv.tv_sec * 1000000) + tv.tv_usec;
...
static int JsonFlowLogger(ThreadVars *tv, void *thread_data, Flow *f)
{
	...
	/* 如果最小范围发送那么调用发送较少字段的函数 */
	if (0 == SC_ATOMIC_GET(flow_conf.flow_all_log_enable)) {
		//OutputJsonBuilderBufferFlow(jb, thread);
	}else {
