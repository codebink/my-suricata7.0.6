 ...
 */
/**
  * 功能：如果指定了default-rule-path，则创建路径
  * 参数sig_file：文件名
  * 返回值：指向字符串path+sig_file的str指针
 */
 ...
 *  参数sig_file：加载规则的文件名
 *  参数goodsigs_tot：文件中有效规则的数量
 *  参数badsigs_tot： 文件中无效规则的数量
 *
 *  返回值：成功时返回 0，错误时返回 -1
 */
static int DetectLoadSigFile( DetectEngineCtx *de_ctx, char *sig_file, int *goodsigs, int *badsigs, int *skippedsigs)
{
    ...
    if (fp == NULL) 
    {
        SCLogError("opening rule file %s: %s.", sig_file, strerror(errno));
        return -1;
    }

    /* 读取规则文件，读取一条规则(包含多行规则)，立即处理一条规则，循环执行，直至读取完文件中的规则 */
    while(fgets(line + offset, (int)sizeof(line) - offset, fp) != NULL)
    {
        ...
        if (line[len] == '\\') 
        {
            ...
            /* No more room in line buffer, continue, rule will fail to parse. */
        }
...
/**
 * 功能：扩展通配符并从每个匹配的文件中读取规则
 *
 * 参数de_ctx：指向检测引擎上下文的指针
 * 参数sig_file:保存签名的文件名（或模式）
 *
 * 返回值: 出错时返回 -1
 */
static int ProcessSigFiles(DetectEngineCtx *de_ctx, char *pattern, SigFileLoaderStat *st, int *good_sigs, int *bad_sigs, int *skipped_sigs)
{
    int r = 0;
 ...
 */
 /**
   ...
   * 
   * 参数de_ctx：指向检测引擎上下文的指针
   * 参数sig_file：保存特征的文件名（或模式）
   * 参数sig_file_exclusive： 传入“sig_file”的文件应以独占方式加载。
   *
   * 返回值: -1（出错时）
   */
int SigLoadSignatures(DetectEngineCtx *de_ctx, char *sig_file, int sig_file_exclusive)
{
    ...
    if (!(sig_file != NULL && sig_file_exclusive == TRUE)) 
    {
        ...
        if (rule_files != NULL)
        {
            ...
            {
                SCLogWarning("Invalid rule-files configuration section: expected a list of filenames.");
            }
            else 
            {
                TAILQ_FOREACH(file, &rule_files->head, next) 
                {
                    /* 如果指定了default-rule-path，则创建路径 */
                    ...
                    {
                        /* Some rules failed to load, just exit as  errors would have already been logged. */
                        exit(EXIT_FAILURE);
                    }
    ...
    if (sig_stat->good_sigs_total <= 0)
    {
        ...
        {
            SCLogWarning("%d rule files specified, but no rules were loaded!", sig_stat->total_files);
        } 
        else 
        {
    ...
    else
    {
        ...
        SCLogInfo("%" PRId32 " rule files processed. %" PRId32 " rules successfully loaded, %" PRId32 " rules failed, %" PRId32,
        ...
        {
            SCDetectRequiresStatusLog(de_ctx->requirements, PROG_VER, strlen(de_ctx->config_prefix) > 0 ? de_ctx->tenant_id : 0);
        }
    }
    ...
    /* 注册规则排序函数。函数的注册顺序体现了优先级。第一个注册的函数比其后注册的函数具有更高的优先级。*/
