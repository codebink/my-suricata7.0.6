...
/* "ESMTP" 模式串 */
uint8_t FTP_ESMTP[4] = {0x53,0x4d,0x54,0x50};

/* 配合 sunday 算法获取模式串第一次命中的位置 */
static int FtpFindIndex(uint8_t *patt_str, int patt_len, uint8_t uc_tmp, int *index)
{
...
static int FtpSundayALG(const uint8_t *src, uint32_t src_len, uint8_t *patt_str, int patt_len, uint32_t *dst_len)
{
    ...
    while (i < src_len) {
        ...
        if ( src[i] == patt_str[j]) {
            ...
            index = 0;
            ret = FtpFindIndex(patt_str, patt_len, src[tmp], &index);
            if ( ret == 1) {
                /* 未找到位置后移 */
                i = tmp + 1;
                j = 0;
...
/* 协议识别函数，识别请求和响应两个方向的数据 */
static AppProto FtpProbingParserTsTc(Flow *f, uint8_t direction, const uint8_t *input, uint32_t input_len, uint8_t *rdir)
{
	if (NULL == f || NULL == input || 0 >= input_len) {
	...
	if (5 <= input_len) {
		FtpSundayALG(input, input_len, FTP_ESMTP, 4, &dst_len);
		if (0 != dst_len) {
			return ALPROTO_UNKNOWN;
		}
	}

	/* 如果没有打开则强制不再识别, 后续的逻辑将不再处理 */
	if (25 == f->dp || 25 == f->sp) {
		return ALPROTO_UNKNOWN;
	}
...
/* 安装请求方向多种特征的协议识别函数 */
static int FtpTcpRegisterPatternsForProtocolDetection(void)
{
	/* "220 (" 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|32 32 30 20|", 4, 0,
		STREAM_TOCLIENT, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}

	/* "FEAT" 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|46 45 41 54|", 4, 0,
		STREAM_TOSERVER, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}

	/* "USER " 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|55 53 45 52 20|", 5, 0,
		STREAM_TOSERVER, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}	

	/* "PASS " 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|50 41 53 53 20|", 5, 0,
		STREAM_TOSERVER, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}	
	
	/* "PORT " 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|50 4f 52 54 20|", 5, 0,
		STREAM_TOSERVER, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}
	
	/* "AUTH SSL" 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|41 55 54 48 20 53 53 4c|", 8, 0,
		STREAM_TOSERVER, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}
	
	/* "AUTH TLS" 特征匹配 */
	if (AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_FTP, "|41 55 54 48 20 54 4c 53|", 8, 0,
		STREAM_TOSERVER, FtpProbingParserTsTc, 0, 0) < 0) {
		return -1;
	}
...
static int FTPRegisterPatternsForProtocolDetection(void)
{
	...
	if (FtpTcpRegisterPatternsForProtocolDetection() < 0) {
		return -1;
	}
