...
typedef struct LogFileCtx_
{
    ...
    /**< Used by some alert loggers like the unified ones that append the date onto the end of files. */
    char *prefix;
    size_t prefix_len;
    ...
    /* Flag if file is a regular file or not.  Only regular files allow for rotation. */
    uint8_t is_regular;
    ...
    /* Socket types may need to drop events to keep from blocking Suricata. */
    uint64_t dropped;

    uint64_t output_errors;

	/* 加一个 tcp 外发时，独立使用的互斥锁，防止多线程外发时导致竞争 */
	SCMutex tcp_mutex;
	uint8_t tcp_mutex_key;
...
int LogFileFreeTcpCtx(LogFileCtx *);
