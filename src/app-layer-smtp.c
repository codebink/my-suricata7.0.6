...
/* MIME_B_ENCODING(0x3F,0x42,0x3F) ?B? 模式串 */
static uint8_t MIME_B_ENCODING_PATT[3] = {0x3F, 0x42, 0x3F};
...
static int SmtpGbkToUtf8(const unsigned char *gbk, int gbken, char *utf8, int *len) {
...
static int MimeFindIndex(uint8_t *patt_str, int patt_len, uint8_t uc_tmp, int *index)
{
...
static int MimeSundayALG(const uint8_t *src, uint32_t src_len, uint8_t *patt_str, int patt_len, uint32_t *dst_len)
{
    ...
    while (i < src_len) {
        ...
        if ( src[i] == patt_str[j]) {
            ...
            index = 0;
            ret = MimeFindIndex(patt_str, patt_len, src[tmp], &index);
            if ( ret == 1) {
                /* 未找到位置后移 */
                i = tmp + 1;
                j = 0;
        ...
        MimeDecParseState *state)
{
    ...
    if (entity->ctnt_flags & CTNT_IS_ATTACHMENT) {
        ...
        if (state->body_begin) {
			...
			/* 判断 filename 是否是 base64 编码 */
			name_len = (int)flen;
			if ((7 < name_len) 
				&& (0x3D == (entity->filename)[0] && 0x3F == (entity->filename)[1] && 0x3F == (entity->filename)[name_len - 2] && 0x3D == (entity->filename)[name_len - 1])) {

				/* base64 模式串 ?B? 匹配      */
				location = 0;
				alg_ret = MimeSundayALG(entity->filename, name_len, MIME_B_ENCODING_PATT, 3, &location);
				if (0 == alg_ret && 0 != location) {
					/* 忽略大小写比较编码 */
					if ((name_len < location + 3 + 2)
						|| ((6 <= (location - 2) && 0 != strncasecmp("gb2312", (char *)(entity->filename + 2), 6))
						&& (3 <= (location - 2) && 0 != strncasecmp("gbk", (char *)(entity->filename + 2), 3)))) {
						goto raw_filename;
					}

					/* 如果是 gb2312 或 gbk 编码 */
					if ((6 <= (location - 2) && 0 == strncasecmp("gb2312", (char *)(entity->filename + 2), 6))
						|| (3 <= (location - 2) && 0 == strncasecmp("gbk", (char *)(entity->filename + 2), 3))) {
						
						/* 分配解码后 filename 使用空间 */
						final_name = (char *)calloc((name_len - location - 3 - 2) * 2, sizeof(char));
						if (NULL == final_name) {
							goto raw_filename;
						}						

						/* 解码 base64 filename */
						Base64Ecode ecode = DecodeBase64((uint8_t *)final_name, ((name_len - location - 3 - 2) * 2), 
											(const uint8_t *)(entity->filename + location + 3), (name_len - location - 3 - 2), 
											&consumed_bytes, &decoded_bytes, 
											BASE64_MODE_RFC4648);
						if (BASE64_ECODE_OK != ecode) {
							SMTP_FREESTR(final_name);
							goto raw_filename;
						}

						/* 创建 utf-8 缓冲区 */
						tmp_len = strlen(final_name);
						tmp_filename = (char *)calloc(tmp_len * 3 + 2, sizeof(char));
						...
						/* 转换成 UTF-8 */
						utf8_ret = SmtpGbkToUtf8((const unsigned char *)final_name, tmp_len, (char *)tmp_filename, &tmp_filename_len);
						if (0 != utf8_ret) {
							SMTP_FREESTR(final_name);
							SMTP_FREESTR(tmp_filename);
							goto raw_filename;
						}

						/* 文件放入容器 */
						if (FileOpenFileWithId(files, &smtp_config.sbcfg, smtp_state->file_track_id++,
									(uint8_t *)tmp_filename, tmp_filename_len, (uint8_t *)chunk, len, flags) != 0) {
