...
static void AlertJsonMariadb(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
	MariadbState *mariadb_state = (MariadbState *)FlowGetAppState(f);
	if (mariadb_state) {
		MariadbTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_MARIADB,
			mariadb_state, tx_id);
		if (tx) {
			JsonBuilderMark mark = { 0, 0, 0 };
			jb_get_mark(js, &mark);
			...
			if (tx->is_request && tx->done) {
				jb_open_object(js, "request");
				JsonMariadbLogRequest(js, tx, mariadb_state);
				jb_close(js);
				logged = true;
			}
			if (!tx->is_request && tx->done) {
				jb_open_object(js, "response");
				JsonMariadbLogResponse(js, tx, mariadb_state);
				jb_close(js);
				logged = true;
			}
			if (logged) {
				/* Close mariadb object. */
				jb_close(js);
			} else {
				jb_restore_mark(js, &mark);
			}
...
static void AlertJsonMysql(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    MysqlState *mysql_state = (MysqlState *)FlowGetAppState(f);
    if (mysql_state) {
        MysqlTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_MYSQL,
            mysql_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonMysqlLogRequest(js, tx, mysql_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonMysqlLogResponse(js, tx, mysql_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
...
static void AlertJsonDb2(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    Db2State *db2_state = (Db2State *)FlowGetAppState(f);
    if (db2_state) {
        Db2Transaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_DB2,
            db2_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonDmdb(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    DmdbState *dmdb_state = (DmdbState *)FlowGetAppState(f);
    if (dmdb_state) {
        DmdbTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_DMDB,
            dmdb_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonDmdbLogRequest(js, tx, dmdb_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonDmdbLogResponse(js, tx, dmdb_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
...
static void AlertJsonTidb(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    TidbState *tidb_state = (TidbState *)FlowGetAppState(f);
    if (tidb_state) {
        TidbTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_TIDB,
            tidb_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonTidbLogRequest(js, tx, tidb_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonTidbLogResponse(js, tx, tidb_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
...
static void AlertJsonHana(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    HanaState *hana_state = (HanaState *)FlowGetAppState(f);
    if (hana_state) {
        HanaTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_HANA,
            hana_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonKingbase(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    KingbaseState *kingbase_state = (KingbaseState *)FlowGetAppState(f);
    if (kingbase_state) {
        KingbaseTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_KINGBASE,
            kingbase_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            bool logged = false;
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonKingbaseLogRequest(js, tx, kingbase_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonKingbaseLogResponse(js, tx, kingbase_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
                /* Close kingbase object. */
                jb_close(js);
            } else {
                jb_restore_mark(js, &mark);
            }
...
static void AlertJsonCassandra(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    CassandraState *cassandra_state = (CassandraState *)FlowGetAppState(f);
    if (cassandra_state) {
        CassandraTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_CASSANDRA,
            cassandra_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            bool logged = false;
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonCassandraLogRequest(js, tx, cassandra_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonCassandraLogResponse(js, tx, cassandra_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
...
static void AlertJsonRadius(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    RadiusState *radius_state = (RadiusState *)FlowGetAppState(f);
    if (radius_state) {
        RadiusTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_RADIUS,
            radius_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonRedis(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    RedisState *redis_state = (RedisState *)FlowGetAppState(f);
    if (redis_state) {
        RedisTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_REDIS,
            redis_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonRedisLogRequest(js, tx, redis_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonRedisLogResponse(js, tx, redis_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
...
static void AlertJsonMongodb(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    MongodbState *mongodb_state = (MongodbState *)FlowGetAppState(f);
    if (mongodb_state) {
        MongodbTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_MONGODB,
            mongodb_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonKrb5(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    Krb5State *krb5_state = (Krb5State *)FlowGetAppState(f);
    if (krb5_state) {
        Krb5Transaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_KRB5,
            krb5_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonLdap(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    LdapState *ldap_state = (LdapState *)FlowGetAppState(f);
    if (ldap_state) {
        LdapTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_LDAP,
            ldap_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonGitsmart(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    GitsmartState *gitsmart_state = (GitsmartState *)FlowGetAppState(f);
    if (gitsmart_state) {
        GitsmartTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_GITSMART,
            gitsmart_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            bool logged = false;
...
static void AlertJsonSqlserver(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    SqlserverState *sqlserver_state = (SqlserverState *)FlowGetAppState(f);
    if (sqlserver_state) {
        SqlserverTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_SQLSERVER,
            sqlserver_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            bool logged = false;
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonSqlserverLogRequest(js, tx, sqlserver_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonSqlserverLogResponse(js, tx, sqlserver_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
                /* Close sqlserver object. */
                jb_close(js);
            } else {
                jb_restore_mark(js, &mark);
            }
...
static void AlertJsonOracle(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    OracleState *oracle_state = (OracleState *)FlowGetAppState(f);
    if (oracle_state) {
        OracleTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_ORACLE,
            oracle_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
...
static void AlertJsonPgsql(const Flow *f, const uint64_t tx_id, JsonBuilder *js)
{
    PgsqlState *pgsql_state = (PgsqlState *)FlowGetAppState(f);
    if (pgsql_state) {
        PgsqlTransaction *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_PGSQL,
            pgsql_state, tx_id);
        if (tx) {
            JsonBuilderMark mark = { 0, 0, 0 };
            jb_get_mark(js, &mark);
            ...
            if (tx->is_request && tx->done) {
                jb_open_object(js, "request");
                JsonPgsqlLogRequest(js, tx, pgsql_state);
                jb_close(js);
                logged = true;
            }
            if (!tx->is_request && tx->done) {
                jb_open_object(js, "response");
                JsonPgsqlLogResponse(js, tx, pgsql_state);
                jb_close(js);
                logged = true;
            }
            if (logged) {
        ...
        const uint32_t sig_flags, const uint16_t option_flags)
{
    ...
    switch (proto) {
        ...
        case ALPROTO_DNP3:
            ...
            break;
        case ALPROTO_MYSQL:
            AlertJsonMysql(p->flow, tx_id, jb);
            break;
        case ALPROTO_MARIADB:
            AlertJsonMariadb(p->flow, tx_id, jb);
            break;
        case ALPROTO_LDAP:
            AlertJsonLdap(p->flow, tx_id, jb);
            break;
        case ALPROTO_DB2:
            AlertJsonDb2(p->flow, tx_id, jb);
            break;
        case ALPROTO_DMDB:
            AlertJsonDmdb(p->flow, tx_id, jb);
            break;
        case ALPROTO_TIDB:
            AlertJsonTidb(p->flow, tx_id, jb);
            break;
        case ALPROTO_HANA:
            AlertJsonHana(p->flow, tx_id, jb);
            break;
        case ALPROTO_KINGBASE:
            AlertJsonKingbase(p->flow, tx_id, jb);
            break;
        case ALPROTO_CASSANDRA:
            AlertJsonCassandra(p->flow, tx_id, jb);
            break;
        case ALPROTO_RADIUS:
            AlertJsonRadius(p->flow, tx_id, jb);
            break;
        case ALPROTO_REDIS:
            AlertJsonRedis(p->flow, tx_id, jb);
            break;
        case ALPROTO_MONGODB:
            AlertJsonMongodb(p->flow, tx_id, jb);
            break;
        case ALPROTO_KRB5:
            AlertJsonKrb5(p->flow, tx_id, jb);
            break;
        case ALPROTO_GITSMART:
            AlertJsonGitsmart(p->flow, tx_id, jb);
            break;
        case ALPROTO_SQLSERVER:
            AlertJsonSqlserver(p->flow, tx_id, jb);
            break;
        case ALPROTO_PGSQL:
            AlertJsonPgsql(p->flow, tx_id, jb);
            break;
        case ALPROTO_ORACLE:
            AlertJsonOracle(p->flow, tx_id, jb);
            break;
        case ALPROTO_HTTP2:
