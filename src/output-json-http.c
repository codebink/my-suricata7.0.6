...
static void EveHttpLogJSONExtended(JsonBuilder *js, htp_tx_t *tx)
{
    ...
    if (h_referer != NULL) {
        ...
        jb_set_string(js, "http_refer", "null");
    }
    ...
    if (tx->response_status != NULL) {
        ...
        if (h_location != NULL) {
            ...
            jb_set_string(js, "redirect", "null");
        }
    }
...
//该函数最终调用位置为 output-tx.c 中 logger->LogFunc() .
static int JsonHttpLogger(ThreadVars *tv, void *thread_data, const Packet *p, Flow *f, void *alstate, void *txptr, uint64_t tx_id)
{
	...
	/* BVM 一期为了适应旧格式，自己构建json日志，二期需要接口和前端完全适应原生代码的消息格式，否则所有事件无法关联 */
	if (0 == SC_ATOMIC_GET(http_conf.http_all_log_enable)) {
		//这里是 一期，现在已废弃，设置 BVM req /resp , base64(body + header) to js.
		//GetTimeStamp(p, f, js); //计算时间戳 
		//ConstructBvmHttpReqRespBase64(p, tx, js, tx_id);
		//OutputJsonBuilderBufferHttp(js, jhl->ctx);	 
	}else {
		//这里是 二期独立函数，需要前端和接口适应原生格式，仅在下面原生格式的基础上，添加 开始时间，结束时间，请求体，响应体，4 个字段，其他不变
        ConstructBvmHttpReqRespBase64V2(p, tx, js, tx_id); //加入http 中时间戳，请求响应base64.
		OutputJsonBuilderBufferHttpAll(js, jhl->ctx);
	}
