...
static int SyslogInit(ConfNode *conf, bool threaded, void **init_data)
{
    ...
    if (facility == -1)
    {
        SCLogWarning("Invalid syslog facility: \"%s\", now using \"%s\" as syslog facility", facility_s, DEFAULT_ALERT_SYSLOG_FACILITY_STR);
    ...
    /* if null we just pass that to openlog, which will then  figure it out by itself. */
