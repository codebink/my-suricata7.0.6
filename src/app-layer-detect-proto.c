...
{
    SCEnter();
    SCLogDebug("buflen %u for %s direction", buflen, (flags & STREAM_TOSERVER) ? "toserver" : "toclient");

    AppProto alproto = ALPROTO_UNKNOWN;     // 定义返回值
    AppProto pm_alproto = ALPROTO_UNKNOWN;  // 记录PM模式获取的协议号

    if (!FLOW_IS_PM_DONE(f, flags))
    {
        AppProto pm_results[ALPROTO_MAX];
        /* 通过特征串模式匹配协议 */
        uint16_t pm_matches = AppLayerProtoDetectPMGetProto(tctx, f, buf, buflen, flags, pm_results, reverse_flow);
        if (pm_matches > 0)
        {
            DEBUG_VALIDATE_BUG_ON(pm_matches > 1);
            alproto = pm_results[0]; // 获取检测结果

            // rerun probing parser for other direction if it is unknown
            // 如果未检测出应用层协议（即应用层协议未知），则重新运行其他方向的探测解析器
            ...
            /* HACK: if detected protocol is dcerpc/udp, we run PP as well to avoid misdetecting DNS as DCERPC. */
            /* HACK：如果检测到的协议是dcerpc/udp，我们也运行PP模式，以避免误将DNS检测为dcerpc。*/
            if (!(ipproto == IPPROTO_UDP && alproto == ALPROTO_DCERPC))
            {
                goto end;
            }

            pm_alproto = alproto; //为PM模式协议号赋值

            /* fall through */
        }
    }

    if (!FLOW_IS_PP_DONE(f, flags)) // 尝试为这个流调用探测解析器
    {
        bool rflow = false;
        alproto = AppLayerProtoDetectPPGetProto(f, buf, buflen, ipproto, flags, &rflow); // 尝试为这个流调用探测解析器
        if (AppProtoIsValid(alproto))  // 如果alproto协议号有效
        {
            if (rflow)
            {
                *reverse_flow = true;
