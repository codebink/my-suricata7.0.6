...
static int SigParseOptions(DetectEngineCtx *de_ctx, Signature *s, char *optstr, char *output, size_t output_size, bool requires)
{
    SigTableElmt *st = NULL;
    char *optname = NULL;
    char *optvalue = NULL;
    ...
    *(optend++) = '\0';
    ...
    /* Check for options that are only to be processed during the first "requires" pass. */
    /**
     *  检查只在第一次“要求”传递期间处理的选项。 
     *  传入requires为true时： 只有requires和sid选项，才会执行选项解析；其他选项不执行选项解析
     *  传入requires为false时：只有requires和sid选项，才不执行选项解析；其他选项均执行选项解析
     */
    ...
    if (!(st->flags & (SIGMATCH_NOOPT|SIGMATCH_OPTIONAL_OPT)))
    {
        ...
        {
            SCLogError("invalid formatting or malformed option to %s keyword: '%s'", optname, optstr);
            goto error;
        }
    } 
...
#define URL "https://suricata.io/our-story/deprecation-policy/"
        if (st->alternative == 0)
        {
            SCLogWarning("keyword '%s' is deprecated and will be removed soon. See %s", st->name, URL);
        }
        else
        {
            SCLogWarning("keyword '%s' is deprecated and will be removed soon. Use '%s' instead. See %s", st->name, sigmatch_table[st->alternative].name, URL);
        }
#undef URL
    }
    ...
    if (optvalue != NULL && strlen(optvalue) > 0)
    {
        ...
        {
            SCLogError("invalid formatting to %s keyword: value must be double quoted \'%s\'", optname, optstr);
            goto error;
        }

        if ((st->flags & (SIGMATCH_QUOTES_OPTIONAL|SIGMATCH_QUOTES_MANDATORY)) && ovlen && *ptr == '"')
        {
            for (; ovlen > 0; ovlen--)
            {
            ...
            {
                SCLogError("bad option value formatting (possible missing semicolon) for keyword %s: \'%s\'", optname, optvalue);
                goto error;
            }
            if (ovlen > 1) 
            {
            ...
            if (ovlen == 0)
            {
                SCLogError("bad input for keyword %s: \'%s\'", optname, optvalue);
                goto error;
            }
        } 
        else 
        {
            if (*ptr == '"')
            {
                SCLogError("quotes on %s keyword that doesn't support them: \'%s\'", optname, optstr);
                goto error;
            }
        }
        
        /* setup may or may not add a new SigMatch to the list */
        /* setup可能会也可能不会将新的SigMatch添加到列表中 */
        setup_ret = st->Setup(de_ctx, s, ptr);
    } 
    else 
    {
        /* setup may or may not add a new SigMatch to the list */
        /* setup可能会也可能不会将新的SigMatch添加到列表中, 此处调用对应应用层协议检测源码中注册的sigmatch_table[DETECT_APPID].Setup */
...
/**
 * 内部函数
 * 简介：将规则字符串拆分成几个块，并分别进一步解析
 *
 * 参数 scan_only：仅扫描，不验证
 */

static int SigParseBasics(DetectEngineCtx *de_ctx, Signature *s, const char *sigstr, SignatureParser *parser, uint8_t addrs_direction, bool scan_only)
{
    ...
    /* 解析方向 */
    if (strcmp(parser->direction, "<>") == 0)  // 签名具有双向运算符
    {
        s->init_data->init_flags |= SIG_FLAG_INIT_BIDIREC;
    }
    else if (strcmp(parser->direction, "->") != 0)
    {
        SCLogError("\"%s\" is not a valid direction modifier, \"->\" and \"<>\" are supported.",parser->direction);
        goto error;
    }
 ...
 *  参数 de_ctx 检测引擎ctx
 *  参数 s        存储规则的内存数据结构
 *  参数 sigstr 原始规则字符串，以空字符结尾
 *  参数 addrs_direction 方向（用于双向规则）
 *  参数 require 仅需要扫描规则
 *
 *  返回值 -1 解析错误
 *  返回值 0     成功
...
static int SigParse(DetectEngineCtx *de_ctx, Signature *s, const char *sigstr, uint8_t addrs_direction, SignatureParser *parser, bool requires)
{
    SCEnter();
    ...
    /**
     * 将规则字符串拆分成几个块，并进一步解析
     * 主要解析action、protocol、src、dst、sp、dp和direction, 赋值给Signature结构体中对应的值
     */
    ...
    if (strlen(parser->opts) > 0)
    {
        ...
        /* loop the option parsing. Each run processes one option and returns the rest of the option string through the output variable. */
        /* 循环执行选项解析。每次运行处理一个选项，赋值给Signature结构体中对应的值，然会通过输出变量返回选项字符串的其余部分 */
        do
        {
...
static Signature *SigInitHelper(DetectEngineCtx *de_ctx, const char *sigstr, uint8_t dir)
{
     ...
     * keyword. */
    /**
     * 根据需求，或仅扫描模式下，对规则进行第一次解析
     * 会发现语法错误，但是完全验证规则的唯一部分是“requires”关键字
     */
     ...
     * only consider the app layer */
     /* 设置数据包和应用层标志，但仅当应用层标志尚未设置时，在这种情况下我们只考虑应用层 */
    ...
    SCLogDebug("sig %"PRIu32" SIG_FLAG_APPLAYER: %s, SIG_FLAG_PACKET: %s",  sig->id, sig->flags & SIG_FLAG_APPLAYER ? "set" : "not set",
 ...
 * \param sigstr Pointer to a character string containing the signature to be parsed.
 *
 * \retval Pointer to the Signature instance on success; NULL on failure.
 */
 /**
   * 功能: 解析签名并将其添加到检测引擎上下文。
   *
   * 参数de_ctx: 指向检测引擎上下文的指针。
   * 参数sigstr: 指向包含待解析规则的字符串的指针。
   *
   * 返回值: 成功时指向签名实例的指针；失败时返回 NULL。
   */
Signature *SigInit(DetectEngineCtx *de_ctx, const char *sigstr)
{
    ...
    if (sig->init_data->init_flags & SIG_FLAG_INIT_BIDIREC) 
    {
        ...
        {
            SCLogInfo("Rule with ID %u is bidirectional, but source and destination are the same, treating the rule as unidirectional", sig->id);
 ...
 */
 /**
   * 功能：解析签名并将其附加到检测引擎上下文规则列表中。
   *
   * 说明：如果规则是双向的，则应将两个规则（地址互换）附加到列表中。
   *       同时处理重复规则。如果规则重复，请使用最新版本的签名。我们使用 sid 和 msg 来识别重复规则。如果两个规则具有相同的 sid 和 gid，则它们为重复规则。
   *
   * 参数 de_ctx：     指向检测引擎上下文的指针。
   * 参数 sigstr：     指向包含要解析规则的字符串的指针。
   * 参数 sig_file：指向包含从中读取规则的文件名的字符串的指针。
   * 参数 lineno：     读取签名的行号。
   *
   * 返回值：指向检测引擎ctx sig_list中规则头的指针（成功时为 NULL）；失败时为 NULL。
   */
Signature *DetectEngineAppendSig(DetectEngineCtx *de_ctx, const char *sigstr)
{
    /* 解析签名并将其添加到检测引擎上下文（重要函数） */
    ...
    /* a duplicate signature that should be chucked out.  Check the previously called function details to understand the different return values */
    /* 重复的规则应该被丢弃。检查之前调用的函数的详细信息，以了解不同的返回值 */
    if (dup_sig == 1) 
    {
    ...
    {
        SCLogWarning("Signature with newer revision, so the older sig replaced by this new signature \"%s\"", sigstr);
    }
    ...
    /** 
     * 在DetectEngineAppendSig()中，规则被添加到前面，总是返回第一个规则。
     * 因此如果规则是双向的，则返回的规则将通过"next"指针指向具有交换地址的克隆规则
     */
