...
{
    // Custom case for only signature-only protocol so far，目前仅适用于签名协议的自定义案例
    if (alproto == ALPROTO_HTTP) 
    {
        return AppLayerParserSupportsFiles(ipproto, ALPROTO_HTTP1) || AppLayerParserSupportsFiles(ipproto, ALPROTO_HTTP2);
    }
...
void AppLayerParserRegisterProtocolParsers(void)
{
	...
	RegisterMariadbParsers();
    RegisterDb2Parsers();
    RegisterDmdbParsers();
    RegisterTidbParsers();
    RegisterHanaParsers();
    RegisterKingbaseParsers();
    RegisterCassandraParsers();
    RegisterRadiusParsers();
    RegisterRedisParsers();
    RegisterMongodbParsers();
    RegisterKrb5ParsersTcp();
    RegisterKrb5ParsersUdp();
    RegisterLdapParsers();
    RegisterGitsmartParsers();
    RegisterSqlserverParsers();
    RegisterPgsqlParsers();
    RegisterOracleParsers();
