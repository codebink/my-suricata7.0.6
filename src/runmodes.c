...
/** \internal
 *  \brief add Sub RunModeOutput to list for Submodule so we can free the output ctx at shutdown and unix socket reload */
/**
 * 内置函数
 * 将Sub RunModeOutput添加到output_free_list列表中，以便我们在关闭和重新加载Unix套接字时释放output ctx.
 */
...
·   {
    ...
    /**
     * 对于注册的所有root日志大类：数据包(packet)日志、文件(filedata和filelogger)日志、事务(tx)日志和流(streaming)日志
     * 将根据输出模块的函数处理指针类型，将模块挂在的各个root日志大类源文件中定义的静态list列表中。以便后续各个日志大类的日志输出函数，遍历调用list列表中的输出模块的输出函数。
     */
    // packet logger
    if (module->PacketLogFunc) 
    {
        ...
        OutputRegisterPacketLogger( module->logger_id, module->name, module->PacketLogFunc, module->PacketConditionFunc, output_ctx,
                                    module->ThreadInit, module->ThreadDeinit, module->ThreadExitPrintStats);
    } 
    //tx logger
    else if (module->TxLogFunc) 
    {
        ...
        OutputRegisterTxLogger( module->logger_id, module->name, module->alproto, module->TxLogFunc, output_ctx, 
                                module->tc_log_progress, module->ts_log_progress, module->TxLogCondition,
                                module->ThreadInit, module->ThreadDeinit, module->ThreadExitPrintStats );
    ...
    else if (module->FiledataLogFunc) 
    {
        ...
        OutputRegisterFiledataLogger(module->logger_id, module->name, module->FiledataLogFunc, output_ctx, 
                                     module->ThreadInit, module->ThreadDeinit, module->ThreadExitPrintStats);
    ...
    else if (module->StreamingLogFunc) 
    {
        ...
        OutputRegisterStreamingLogger(module->logger_id, module->name, module->StreamingLogFunc, output_ctx, module->stream_type,
                                      module->ThreadInit, module->ThreadDeinit, module->ThreadExitPrintStats);
    } 
    else
    {
...
/* 循环获取output_modules内容,填充到对应root大类日志中的list里 */
static void RunModeInitializeEveOutput(ConfNode *conf, OutputCtx *parent_ctx)
{
    ...
    TAILQ_FOREACH(type, &types->head, next)
    {
        ...
        {
            // 使用子输出模块的配置节点名称，与配置节点名称进行匹配。
            // 如果相同，则继续处理；否则继续进行下一次循环 */
            if (strcmp(subname, sub_module->conf_name) == 0) 
            {
                sub_count++;

                // 父类输出模块名称不为eve-log, 则报错
                if (sub_module->parent_name == NULL || strcmp(sub_module->parent_name, "eve-log") != 0)
                {
                    FatalError("bad parent for %s", subname);
                }
                 // 子类输出模块初始化函数为NULL, 则报错
                if (sub_module->InitSubFunc == NULL)
                {
                    FatalError("bad sub-module for %s", subname);
                }

                /* pass on parent output_ctx，传递parent    output_ctx */
                /* 执行eve-log日志模块中的子类输出模块输出函数, 如：OutputFTPLogInitSub、JsonAlertLogInitCtxSub */
                ...
                /* 将eve-log子类输出模块添加到释放列表output_free_list中 */
        ...
        /* 在output_modules中没有找到名称为subname的输出模块时, 进行报错 */
        if (!sub_count)
        {
...
void RunModeInitializeOutputs(void)
{
    ...
    TAILQ_FOREACH(output, &outputs->head, next) 
    {
        ...
        // file-log不再支持，使用eve.files替代
        if (strcmp(output->val, "file-log") == 0)
        {
            SCLogWarning("file-log is no longer supported, use eve.files instead (see ticket #2376  for an explanation)");
            continue;
        }
        // Unified1不再支持
        else if (strncmp(output->val, "unified-", sizeof("unified-") - 1) == 0)
        {
            SCLogWarning("Unified1 is no longer supported, use Unified2 instead (see ticket #353 for an explanation)");
            continue;
        }
        // Unified2不再支持
        ...
        {
#ifndef HAVE_LUA
            // 使用宏定义HAVE_LUA判断编译时是否添加了Lua支持。如果编译时未添加Lua支持，但是配置时却配置了lua输出，则报错。
            // lua支持未编译。使用 lua(jit) 及其安装的开发文件重新配置/重新编译以添加 lua 支持
            SCLogWarning("lua support not compiled in. Reconfigure/recompile with lua(jit) and its development files installed to add lua support.");
            continue;
#endif
        }
        // 从 Suricata5.0开始，dns-log不再可用
        ...
        /* 遍历输出模块列表output_modules，将具体的输出模块挂在到对应日志大类的静态list中 */
        OutputModule *module;
        int count = 0;
        TAILQ_FOREACH(module, &output_modules, entries) 
        {
            // 使用输出模块中的配置节点名称，与配置节点名称进行匹配。
            // 如果相同，则继续处理；否则继续进行下一次循环
            ...
            {
                // 对于eve-log输出模块, 此处会调用OutputJsonInitCtx
            ...
            // 对于父类输出模块，需查找它的子类输出模块
            // eve-log
            if (strcmp(output->val, "eve-log") == 0) 
            {
                /* 初始化eve-log所有子类输出模块，将具体的一个output_module, 填充到对应大类日志中的list里 */
                RunModeInitializeEveOutput(output_config, output_ctx);

                /* add 'eve-log' to free list as it's the owner of the main output ctx from which the sub-modules share the  LogFileCtx */
                /* 将eve-log父类输出模块添加到释放列表output_free_list中，因为它是main output ctx  的所有者，子模块共享LogFileCtx */
            ...
            else if (strcmp(output->val, "lua") == 0)
            {
                ...
                /* 初始化lua输出模块，将具体的一个output_module, 填充到对应大类日志中的list里 */
    ...
    {
        /* old YAML with no "tls-store" in outputs. "tls-log" value needs to be started using 'tls-log' config as own config */
        /* 旧的yaml文件中的outputs配置节点中, 没有tls-store子配置节点。“tls-log”值需要使用“tls-log”配置作为自己的配置来启动 */
    ...
    for (a = 0; a < ALPROTO_MAX; a++) 
    {
        if (AppLayerParserSupportsFiles(IPPROTO_TCP, a))
        {
            ...
            SCLogDebug("IPPROTO_TCP::%s: g_file_logger_enabled %d g_filedata_logger_enabled %d -> %08x", AppProtoToString(a), g_file_logger_enabled, g_filedata_logger_enabled, logger_bits[a]);
        }
        ...
        const int tcp = AppLayerParserProtocolHasLogger(IPPROTO_TCP, a) | (g_file_logger_enabled) | (g_filedata_logger_enabled);
        const int udp = AppLayerParserProtocolHasLogger(IPPROTO_UDP, a) | (g_file_logger_enabled) | (g_filedata_logger_enabled);
        SCLogDebug("tcp %d udp %d", tcp, udp);

        SCLogDebug("logger for %s: %s %s", AppProtoToString(a), tcp ? "true" : "false", udp ? "true" : "false");
    ...
    // 设置活跃的root大类日志模块至active_loggers列表中
