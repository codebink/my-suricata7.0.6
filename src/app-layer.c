 ...
 *  \retval int 0 ok
 */
/* 解析需要先进行协议的识别，判断出流的协议后，在结构体中设置标志，给后面的解析函数使用，协议识别主要分为三种方式.
 * 函数调用堆栈：*AppLayerHandleTCPData* -> *TCPProtoDetect* （协议类型检测）-> *AppLayerProtoDetectGetProto* ->
 * 1. AppLayerProtoDetectPMGetProto：通过特征串匹配	
 * 2. AppLayerProtoDetectPPGetProto：通过探测方式识别，主动发送报文探测结果
 * 3. AppLayerProtoDetectPEGetProto
 */
        ...
        enum StreamUpdateDir dir)
{
    ...
    /* If a gap notification, relay the notification on to the app-layer if known. */
    /* 如果是一个gap通知，如果知道应用层协议，则将通知转发到应用程序层。 */
    if (flags & STREAM_GAP)
    {
        if (alproto == ALPROTO_UNKNOWN)
        {
    ...
    /* if we don't know the proto yet and we have received a stream initializer message, we run proto detection.
     * We receive 2 stream init msgs (one for each direction), we only run the proto detection for both and emit an event
     * in the case protocols mismatch. */
    /* 如果未识别出协议，并且收到流初始化消息，则进行协议识别。
     * 收到2个流初始化消息(每个方向一个)时，一边只进行一次协议识别。只有在协议未识别时，才会对两个两个方向进行协议识别，且发出一个事件信息
     */
    /* 未识别出应用层协议，并且收到流初始化标志 */
    if (alproto == ALPROTO_UNKNOWN && (flags & STREAM_START)) 
    {
        DEBUG_VALIDATE_BUG_ON(FlowChangeProto(f));
        /* run protocol detection */
        /* 运行协议检测识别，如何检测应用层协议，TCPProtoDetect函数内部调用了应用层数据解析接口AppLayerParserParse */
        if (TCPProtoDetect(tv, ra_ctx, app_tctx, p, f, ssn, stream, data, data_len, flags, dir) != 0)
        {
            goto failure;
        }
    } 
    /* 已识别出应用层协议，但是本条流绑定的协议发生改变 */
    else if (alproto != ALPROTO_UNKNOWN && FlowChangeProto(f))
    {
        ...
        // 延迟AppLayerParserStateCleanup，因为可能需要以前的解析器状态
        ...
        StreamTcpResetStreamFlagAppProtoDetectionCompleted(&ssn->server);
         /* 运行协议检测识别，如何检测应用层协议，TCPProtoDetect函数内部调用了应用层数据解析接口AppLayerParserParse */
        int rd = TCPProtoDetect(tv, ra_ctx, app_tctx, p, f, ssn, stream, data, data_len, flags, dir);
        if (f->alproto == ALPROTO_UNKNOWN)
        {
            DEBUG_VALIDATE_BUG_ON(alstate_orig != f->alstate);
            // not enough data, revert AppLayerProtoDetectReset to rerun detection
            // 数据不足，恢复AppLayerProtoDetectReset操作，以重新运行检测
            f->alparser = alparser;
        ...
        SCLogDebug("protocol change, old %s, new %s", AppProtoToString(f->alproto_orig), AppProtoToString(f->alproto));

        if (f->alproto_expect != ALPROTO_UNKNOWN && f->alproto != ALPROTO_UNKNOWN && f->alproto != f->alproto_expect) 
                {
            ...
            {
                AppLayerDecoderEventsSetEventRaw(&p->app_layer_events, APPLAYER_NO_TLS_AFTER_STARTTLS);
    ...
    {
        SCLogDebug("stream data (len %" PRIu32 " alproto " "%"PRIu16" (flow %p)", data_len, f->alproto, f);
#ifdef PRINT
        if (data_len > 0) 
        {
            printf("=> Stream Data (app layer) -- start %s%s\n", flags & STREAM_TOCLIENT ? "toclient" : "",  flags & STREAM_TOSERVER ? "toserver" : "");
            PrintRawDataFp(stdout, data, data_len);
            printf("=> Stream Data -- end\n");
        }
#endif
        /* if we don't have a data object here we are not getting it a start msg should have gotten us one */
        /* 如果这里没有数据对象，我们就得不到它。一开始的消息应该能给我们一个 */
        if (f->alproto != ALPROTO_UNKNOWN) 
        {
            PACKET_PROFILING_APP_START(app_tctx, f->alproto);
            /* 解析应用层数据 */
