 ...
 *
 * \param de_ctx Pointer to the Detection Engine Context that holds the signatures to be ordered
 */
 /**
   * 对规则进行排序
   *
   * 参数de_ctx: 指向保存待排序特征的检测引擎上下文的指针
   */
...
/**
  * 功能：允许注册规则排序函数。
  *       函数的注册顺序体现了优先级。第一个注册的函数比其后注册的函数具有更高的优先级。
  *       要添加新的注册函数，请根据希望赋予该关键字的优先级，按照以下顺序将其列在正确的位置进行注册。
  *
  * 参数de_ctx: 指向需要对特征规则进行排序的检测引擎上下文的指针。
  */
 ...
 *
 * \param de_ctx Pointer to the detection engine context from which the signatures were ordered.
 */
 /**
   * 功能 注销所有已注册的规则排序函数
   *
   * 参数de_ctx: 指向对规则进行排序的检测引擎上下文的指针。
  */
