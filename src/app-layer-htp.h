...
typedef struct HtpTxUserData_ {
    ...
    HtpBody request_body; //请求body.
    HtpBody response_body;//响应body.
...
typedef struct HtpState_ {
    ...
    htp_connp_t *connp; //http 连接跟踪
    /* Connection structure for each connection 链接. */
    htp_conn_t *conn; //TCP 连接跟踪
    ...
    uint64_t tx_freed;
    const struct HTPCfgRec_ *cfg; //htp配置信息for suricata.yaml http配置信息.
    uint16_t flags;
    uint16_t events;
...
/* Reload 模块开关和日志开关 */
typedef struct HttpConf_ {
	SC_ATOMIC_DECLARE(int, http_enable);
	SC_ATOMIC_DECLARE(int, http_all_log_enable);

	SC_ATOMIC_DECLARE(uint32_t, req_headersize);  //app-layer.protocols.http.request-header-limit
	SC_ATOMIC_DECLARE(uint32_t, resp_headersize); 
    
	SC_ATOMIC_DECLARE(uint32_t, req_bodysize);   //app-layer.protocols.http.request-body-limit
	SC_ATOMIC_DECLARE(uint32_t, resp_bodysize);
}HttpConf;
