...
/** 功能: 打开一个通用的输出“日志文件”，它可以是一个常规文件或套接字
 *
 *   conf:              输出部分的ConfNode结构
 *   log_ctx:           调用者分配的日志文件上下文
 *   default_filename:  要打开的文件的默认名称，如果没有在ConfNode中指定
 *   rotate:            在HUP中注册旋转文件。
 *   
 *  成功时返回0, 错误时返回-1
 */
 ...
 */
/**
 * 功能：LogFileEnsureExists() 确保线程的日志文件上下文存在
 *
 * 参数：parent_ctx
 *
 * 返回值：成功返回LogFileCtx类型指针，失败返回NULL
 */
LogFileCtx *LogFileEnsureExists(LogFileCtx *parent_ctx)
{
    ...
    SCLogDebug("Adding reference for thread %ld [slot %d] to file %s [ctx %p]", SCGetThreadIdLong(), entry->slot_number, parent_ctx->filename, parent_ctx);

    bool new = entry->isopen;
    /* has it been opened yet? */
    if (!entry->isopen)
    {
        SCLogDebug("Opening new file for thread/slot %d to file %s [ctx %p]", entry->slot_number, parent_ctx->filename, parent_ctx);
        if (LogFileNewThreadedCtx(parent_ctx, parent_ctx->filename, parent_ctx->threads->append, entry))
        {
            entry->isopen = true;
        }
        else
        {
            SCLogError( "Unable to open slot %d for file %s", entry->slot_number, parent_ctx->filename);
    ...
    if (sc_log_global_log_level >= SC_LOG_DEBUG) 
    {
        if (new)
        {
            SCLogDebug("Existing file for thread/entry %p reference to file %s [ctx %p]", entry, parent_ctx->filename, parent_ctx);
        }
    }
...
/* 这个函数主要销毁互斥锁 */
int LogFileFreeTcpCtx(LogFileCtx *lf_ctx)
{
	/* 参数检查 */
    if (lf_ctx == NULL) 
    {
...
int LogFileWriteToMgrTcp(LogFileCtx *file_ctx, char *buff, uint32_t buff_len)
{

	SCMutexLock(&file_ctx->tcp_mutex);
		
	/* 通过 TCP连接发送 */
	FlowTcpWrite(file_ctx, buff, buff_len);
		
	SCMutexUnlock(&file_ctx->tcp_mutex);
...
int LogFileWriteToMgr(LogFileCtx *file_ctx, MemBuffer *buffer)
{
	...
	/* 通过 redis 发送 */
	LogFileWriteRedis(file_ctx, (const char *)MEMBUFFER_BUFFER(buffer), MEMBUFFER_OFFSET(buffer));
...
int LogFileWrite(LogFileCtx *file_ctx, MemBuffer *buffer)
{
    ...
    {
        file_ctx->plugin.plugin->Write((const char *)MEMBUFFER_BUFFER(buffer), MEMBUFFER_OFFSET(buffer), file_ctx->plugin.init_data, file_ctx->plugin.thread_data);
    }
#ifdef HAVE_LIBHIREDIS
