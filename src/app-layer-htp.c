...
/* http reload 使用的开关 */
#define HTTP_LOG_NODE "app-layer.protocols.http.all-log"
#define HTTP_ENABLED "app-layer.protocols.http.enabled"

#define HTTP_REQ_HEADERSIZE "app-layer.protocols.http.request-header-limit"
#define HTTP_RESP_HEADERSIZE "app-layer.protocols.http.request-header-limit"
#define HTTP_REQ_BODYSIZE "app-layer.protocols.http.request-body-limit"
#define HTTP_RESP_BODYSIZE "app-layer.protocols.http.response-body-limit"
HttpConf http_conf;

/* 热加载函数 */
void HttpReload(void)
{
	ConfNode *node = NULL;

	/* 获取是否打开日志开关 */
	node = NULL;
	node = ConfGetNode(HTTP_LOG_NODE);
	if (node && node->val != NULL) {
		SC_ATOMIC_SET(http_conf.http_all_log_enable, ConfValIsTrue(node->val));
	}else {
		SC_ATOMIC_SET(http_conf.http_all_log_enable, 0);
	}

	/* 插件开关 */
	node = NULL;
	node = ConfGetNode(HTTP_ENABLED);
	if (node && node->val != NULL) {
		SC_ATOMIC_SET(http_conf.http_enable, ConfValIsTrue(node->val));
	}else {
		SC_ATOMIC_SET(http_conf.http_enable, 0);
	}
//
    uint32_t val=0;
	node = NULL;
	node = ConfGetNode(HTTP_REQ_HEADERSIZE); //req_headersize
	if (node && node->val != NULL) {
        if (ParseSizeStringU32(node->val,  &val) < 0) {
               SCLogError("Error parsing request_headersize wzz "
                          "from conf file - %s.  Killing engine",
                       node->val);
               exit(EXIT_FAILURE);
       }

		SC_ATOMIC_SET(http_conf.req_headersize,  val);
        val=0;
	}else {
		SC_ATOMIC_SET(http_conf.req_headersize, 20480);
	}

    node = NULL;
	node = ConfGetNode(HTTP_RESP_HEADERSIZE); //resp_headersize
	if (node && node->val != NULL) {
        if (ParseSizeStringU32(node->val,  &val) < 0) {
              SCLogError("Error parsing response_headersize wzz "
                         "from conf file - %s.  Killing engine",
                      node->val);
              exit(EXIT_FAILURE);
        }
		SC_ATOMIC_SET(http_conf.resp_headersize,  val);
        val=0;
	}else {
		SC_ATOMIC_SET(http_conf.resp_headersize, 20480);
	}

	node = NULL;
	node = ConfGetNode(HTTP_REQ_BODYSIZE); //req_bodysize.
	if (node && node->val != NULL) {
        if (ParseSizeStringU32(node->val,  &val) < 0) {
             SCLogError("Error parsing req_bodysize wzz "
                        "from conf file - %s.  Killing engine",
                     node->val);
             exit(EXIT_FAILURE);
        }

		SC_ATOMIC_SET(http_conf.req_bodysize,  val);
        val=0;
	}else {
		SC_ATOMIC_SET(http_conf.req_bodysize, 40960);
	}

    node = NULL;
	node = ConfGetNode(HTTP_RESP_BODYSIZE); //resp_bodysize.
	if (node && node->val != NULL) {
        if (ParseSizeStringU32(node->val,  &val) < 0) {
              SCLogError("Error parsing resp_bodysize wzz "
                         "from conf file - %s.  Killing engine",
                      node->val);
              exit(EXIT_FAILURE);
        }
		SC_ATOMIC_SET(http_conf.resp_bodysize,  val);
        val=0;
	}else {
		SC_ATOMIC_SET(http_conf.resp_bodysize, 40960);
	}
}
        ...
        StreamSlice stream_slice, void *local_data)
{
	...
	if (0 == SC_ATOMIC_GET(http_conf.http_enable)) {
    ...
    //赋值请求时间戳for 计算差值.. wzz
    htp_set_timestamp(hstate->connp, 0);
        ...
        StreamSlice stream_slice, void *local_data)
{
    ...
    if (input_len > 0) {
        ...
        //赋值响应时间戳for 计算差值..wzz.
        htp_set_timestamp(hstate->connp, 1);
...
static void HTPConfigSetDefaultsPhase1(HTPCfgRec *cfg_prec)
{
    ...
    htp_config_register_request_body_data(cfg_prec->cfg, HTPCallbackRequestBodyData); //请求响应body.
    htp_config_register_response_body_data(cfg_prec->cfg, HTPCallbackResponseBodyData);//响应body.
...
static AppProto HttpProbingParserTsTc(Flow *f, uint8_t direction, const uint8_t *input, uint32_t input_len, uint8_t *rdir)
{
...
static int HTPRegisterPatternsForProtocolDetection(void)
{
    ...
    for (methods_pos = 0; methods[methods_pos]; methods_pos++) {
        for (spacings_pos = 0; spacings[spacings_pos]; spacings_pos++) {
            ...
            */
            ...
            register_result = AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_HTTP1,
                    method_buffer, (uint16_t)strlen(method_buffer) - 3, 0, STREAM_TOSERVER, 
                    HttpProbingParserTsTc, 0, 0);
    ...
    for (versions_pos = 0; versions[versions_pos]; versions_pos++) {
        ...
        register_result = AppLayerProtoDetectPMRegisterPatternCSwPP(IPPROTO_TCP, ALPROTO_HTTP1,
                versions[versions_pos], (uint16_t)strlen(versions[versions_pos]), 0,
                STREAM_TOCLIENT, HttpProbingParserTsTc, 0, 0);				
        if (register_result < 0) {
            return -1;
        }
    }
...
void RegisterHTPParsers(void)
{
    ...
    if (AppLayerParserConfParserEnabled("tcp", proto_name)) {
        ...
        AppLayerParserRegisterGetTxCnt(IPPROTO_TCP, ALPROTO_HTTP1, HTPStateGetTxCnt); //http tx 
        AppLayerParserRegisterGetTx(IPPROTO_TCP, ALPROTO_HTTP1, HTPStateGetTx);  //http  HtpState->conn->transactions 获取缓存http 请求/响应数据.
        ...
        AppLayerParserRegisterParser(
                IPPROTO_TCP, ALPROTO_HTTP1, STREAM_TOSERVER, HTPHandleRequestData); //http请求解析.
        AppLayerParserRegisterParser(
                IPPROTO_TCP, ALPROTO_HTTP1, STREAM_TOCLIENT, HTPHandleResponseData); //http响应解析
