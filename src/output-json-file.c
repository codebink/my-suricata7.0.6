...
static int FileInfoGetUtime(const SCTime_t ts, uint64_t *utime) 
{
    if (utime == NULL) {
        return -1;
    }
    ...
    // 计算总微秒数
    // 1 秒 = 1,000,000 微秒
    *utime = (ts.secs * 1000000ULL) + ts.usecs;
        ...
        OutputJsonCtx *eve_ctx)
{
    ...
    if (stored) {
        ...
        FileInfoGetUtime(p->ts, &utime);
        jb_set_uint(js, "u_timestamp", utime);
        ...
        void *tx, const uint64_t tx_id, uint8_t dir)
{
	...
	/* 异常判断 文件名为 '/' 或 '//' 的即使有内容也不还原 */
	if (NULL != ff && 1 == ff->name_len && NULL != ff->name && 0x2f == ff->name[0]) {
		return 0;
	}
