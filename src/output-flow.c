...
/* flow reload 使用的开关 */
#define FLOW_LOG_NODE "app-layer.protocols.flow-log.all-log"
#define FLOW_ENABLED "app-layer.protocols.flow-log.enabled"
#define FLOW_ETH_NAME "app-layer.protocols.flow-log.in-iface"
...
void FlowReload(void)
{
	...
	/* 获取是否打开日志开关 */
	node = NULL;
	node = ConfGetNode(FLOW_LOG_NODE);
	if (node && node->val != NULL) {
		SC_ATOMIC_SET(flow_conf.flow_all_log_enable, ConfValIsTrue(node->val));
	}else {
		SC_ATOMIC_SET(flow_conf.flow_all_log_enable, 0);
	}

	/* 插件开关 */
	node = NULL;
	node = ConfGetNode(FLOW_ENABLED);
	if (node && node->val != NULL) {
		SC_ATOMIC_SET(flow_conf.flow_enable, ConfValIsTrue(node->val));
	}else {
		SC_ATOMIC_SET(flow_conf.flow_enable, 0);
	}

	/* 网口名称 */
	node = NULL;
	node = ConfGetNode(FLOW_ETH_NAME);
	if (node && node->val != NULL) {
		if (FLOW_DEV_NAME_LEN <= strlen(node->val)) {
			memcpy(flow_conf.in_iface, "none", 4);
		}else {
			memcpy(flow_conf.in_iface, node->val, strlen(node->val));
		}
	}else {
		memcpy(flow_conf.in_iface, "none", 4);
	}
